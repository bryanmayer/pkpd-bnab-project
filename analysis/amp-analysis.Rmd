...
ic80_pe = read_excel("data/nejm_fig3a.xlsx", col_names = c("ic80", "pe")) %>% mutate(dose = "pooled")
ic80_all = read_csv("data/dose10_scrape.csv", col_names = c("ic80", "pe")) %>% 
  mutate(dose = "30") %>%
   bind_rows(mutate(read_csv("data/dose30_scrape.csv", col_names = c("ic80", "pe")), dose = "10")) %>%
  bind_rows(ic80_pe)
```
...
amp_conc_data = tribble(
  ~dose, ~study, ~n, ~peak, ~midpoint, ~trough,
...
amp_conc_merge = amp_conc_avg %>%
  mutate(dose = "pooled") %>%
  bind_rows(mutate(amp_conc_dose, dose = as.character(dose)))
...
titer_pe = ic80_pe %>%
  ...
  left_join(gather(amp_conc_avg, time, mean_conc, midpoint, peak, trough), by = c("time")) %>%
  mutate(
    id80 = mean_conc/ic80,
    id50 = 4 * id80
  ) 
...
ggplot(titer_pe, aes(x = id80, y = pe, colour = time)) +
  geom_point() +
  geom_line() +
  geom_line(data = neutralization, colour = "black") +
  scale_x_log10() +
...
ic80_all %>%
  ggplot(aes(x = ic80, y = pe, colour = dose)) +
  geom_point() +
  geom_line() +
  scale_x_log10() 
...
titer_pe_dose = ic80_all %>%
  crossing(time = c("peak", "midpoint", "trough")) %>%
  left_join(gather(amp_conc_merge, time, mean_conc, midpoint, peak, trough), by = c("time", "dose")) %>%
  mutate(
    id80 = mean_conc/ic80,
    id50 = 4 * id80
  ) 

ggplot(titer_pe_dose, aes(x = id80, y = pe, colour = dose)) +
  geom_point() +
  geom_line() +
  geom_line(data = neutralization, colour = "black") +
  scale_x_log10() +
...
ggplot(titer_pe_dose, aes(x = id50, y = pe, colour = dose)) +
  geom_point() +
  geom_line() +
  geom_line(data = neutralization, colour = "black") +
  scale_x_log10() +
