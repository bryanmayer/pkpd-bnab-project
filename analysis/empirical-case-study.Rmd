...
full_combo_res %>%
  dplyr::filter(str_detect(outcome, "coverage"))   %>%
  ggplot(aes(x = ratio, y = value, colour = as.factor(parse_number(outcome)), linetype = endpoint)) +
  geom_line() + 
...
single_mab_pl = map(unique(marg_res$endpoint), function(x){
  marg_res %>%
  dplyr::filter(str_detect(outcome, "coverage") & endpoint == x) %>%
  mutate(outcome3 = paste(x, paste0("(",outcome2,")"))) %>%
  ...
  facet_grid(cols = vars(category), rows = vars(outcome3), scale = "free_x", space = "free_x", 
             switch = "both") +
    theme(strip.placement = "outside", strip.background = element_blank(),
...
pl_leg = get_legend(single_mab_pl[[1]])
single_mab_pl[[2]] + theme(legend.position = "none") 

combo_mab_pl = map(unique(marg_res$endpoint), function(x){
  full_combo_res %>%
  dplyr::filter(str_detect(outcome, "coverage") & endpoint == x) %>%
  mutate(analytes2 = str_replace_all(analytes, ":", ":\n")) %>%
  ...
  geom_line() + 
  scale_y_continuous(limits = c(0, 100), breaks = 0:4*25) +
  geom_point(data = dplyr::filter(combo_best_pl, endpoint == x), aes(x = ratio),
             shape = 21, size = 2) +
  facet_grid(cols = vars(analytes2), rows = vars(outcome), 
...
triple_mab_pl = map(unique(marg_res$endpoint), function(x){
  triple_res_opt %>%
  bind_rows(triple_res_fixed) %>%
  ...
  scale_y_continuous(limits = c(0, 100), breaks = 0:4*25) +
  labs(x = "", colour = "", shape = "", y = "% of viruses") +
  facet_grid(cols = vars(category2), rows = vars(outcome), 
             scale = "free_x", space = "free_x", switch = "x") +
  theme(strip.placement = "outside", strip.background = element_blank(),
        legend.position = "none", axis.text.x = element_text(size = 8)) +
  labs(x = "") 
...
combo_pl = plot_grid(single_mab_pl[[2]] + theme(legend.position = "none"), 
          combo_mab_pl[[2]] + yaxis_off, triple_mab_pl[[2]] + yaxis_off,
          axis = "tb",
...
```{r coverage_trough, fig.width = 8}

combo_pl_auc = plot_grid(single_mab_pl[[1]] + theme(legend.position = "none"), 
          combo_mab_pl[[1]] + yaxis_off, triple_mab_pl[[1]] + yaxis_off,
          axis = "tb",
          align = "h", rel_widths = c(0.9, 1.1, 1), nrow = 1)

plot_grid(pl_leg, combo_pl_auc, nrow = 2, rel_heights = c(1.5,12))
