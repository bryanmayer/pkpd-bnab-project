...
Here we will want to look at both mean IIP and coverage IIP > (2 from pegu?)

```{r CS2-setup}
#case study2 is an artifact of old two case study analysis
...
CS2_coverage = case_study2_full %>%
  select(main_vars, contains("coverage")) %>%
  gather(key, value, -main_vars) %>%
  separate(key, 
...
Look at all doses and summary endpoints to start. Switch to 300, 1200ug; mean ipi
...
viral_iC50s = .sample_IC50s(total_virus_sims = 500, pd_parms = bispecific_pd,
                              virus_seed = 822960625) %>%
  mutate(
    minIC50 = pmin(IC50_1, IC50_2),
    maxIC50 = pmax(IC50_1, IC50_2)
    )
  
synergy_iC50s = map_df(c(1, 10, 100, 1000), function(i){
  viral_iC50s %>%
    transmute(
      synergy_factor = i,
      maxNeut = minIC50/i,
  ...
  gather(interaction, IC50, maxNeut, minNeut)
...
bispecific_pkpd = bispecific_pk %>%
    ...
    mutate(
      ID50_dat = map(conc, ~ (
        tibble(
          synergy_factor = synergy_iC50s$synergy_factor,
          interaction = synergy_iC50s$interaction,
          ID50 = .x/synergy_iC50s$IC50,
          neut = titer2neut(ID50),
          iip = -log10(1 - neut), #not mean but named this for later
          coverage = iip > 2  #not mean but named this for later
        ) %>%
...
This analysis is difficult because there aren't that many studies with direct parental vs. bi-specific  comparisons. Even when they exist, PK data is rare. A general summary of 5 bi-specific papers (including Wagh et al papers) is that bi-specifics seem to give an IC50 boost (at a common concentration?) sometimes looking like synergy (>10-fold decrease in IC50) but there are certainly cases where the parent looks better. This could be noise when there isn't synergy, but not a clean story.

The data was sort of a mess, so would want a suggestion from co-authors if this is worth the effort.
